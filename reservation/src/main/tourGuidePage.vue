...
const fetchData = async () => {
    ...
    const authResponse = await axios.get(`${API_URL}api/gettourguide`);
    items.value = authResponse.data;
...
<template>
    <section class="d-flex">
            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="navbar-nav">
                        ...
                        <li class="nav-item">
                            <RouterLink to="/TourguideReservation" class="nav-link">Tourist Reservation</RouterLink>
                        </li>
        ...
        <div v-for="item in items" :key="item.tg_id" class="d-none d-lg-block"
            style="box-shadow: rgba(0, 0, 0, 0.1) 0px 4px 6px -1px, rgba(0, 0, 0, 0.06) 0px 2px 4px -1px;">
            <div v-if="id == item.tg_id" class="d-flex my-2">
                <img :src="'/@fs/xampp/htdocs/reservation/src/assets/uploads/' + item.tg_img" @click="isSet"
                    style="width: 35px;height: 35px;border-radius: 50%;"
                    :data-bs-target="'#bookmodal-' + item.tg_id">
                <div href="" class="d-none d-md-block my-1 text-decoration-none text-dark "
                    ...
                    {{ item.tg_fname }} {{ item.tg_lname }}</div>
                            ...
                            <div class="modal-body">
                                <div class="card text-center">
                                    ...
                                    <div v-if="isImage"
                                        ...
                                        @click="image2">
                                        <img v-if="isImage2"
                                            :src="'/@fs/xampp/htdocs/reservation/src/assets/uploads/' + item.tg_img"
                                            class="wide">
                                    </div>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <strong>Phone:</strong>
                                                <span class="m-2">{{ item.tg_cpnum }}</span>
                                            </div>
                                            <div class="col-md-6">
                                                <strong>Address:</strong>
                                                <span class="m-2">{{ item.tg_address }}</span>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12 d-block">
                                                <strong>{{ item.tg_fname }} {{ item.tg_lname }}</strong>
                                                <div class="m-2 text-muted">{{ item.tg_email }}</div>
                                            </div>
                                        </div>
                                    </div>
